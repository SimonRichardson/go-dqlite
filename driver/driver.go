...
type Driver struct {
	...
	stmtCache             PreparedStatementCache // Cache for prepared statements
}
...
// WithStmtCacheCapacity will set the size of the prepared statement cache. If
// the capacity is set to 0, the cache will be disabled.
func WithStmtCacheCapacity(capacity int) Option {
	return func(options *options) {
		if capacity > 0 {
			options.StmtCache = newStmtCache(capacity)
		} else {
			options.StmtCache = &noopCache{}
		}
	}
}
...
// PrepareOne prepares a single statement and returns it along with the number
// of characters matched from the query.
...
// ExecOne executes a single statement and returns the result along with the
// number of characters matched from the query.
